...
def key_exchange(their_value: int, my_int: int, p:int) -> hex: #A is calculated by opposite party
    shared_key = bytes(str(pow(their_value, my_int, p)).encode("utf8"))         # int -> str -> bytes
...
if __name__ == "__main__":
    ...
    g = formatHex("A4D1CBD5 C3FD3412 6765A442 EFB99905 F8104DD2 58AC507F D6406CFF 14266D31 266FEA1E 5C41564B 777E690F 5504F213 160217B4 B01B886A 5E91547F 9E2749F4 D7FBD7D3 B9A92EE1 909D0D22 63F80A76 A6A24C08 7A091F53 1DBF0A01 69B6A28A D662A4D1 8E73AFA3 2D779D59 18D08BC8 858F4DCE F97C2A24 855E6EEB 22B3B2E5")
    ...
    print("Verifying key exchange - are keys same?", alice_key == bob_key, "\n")

    a_msg = create_cipher(pad(b"hi bob", BLOCK_SIZE), str(alice_key), iv)
    b_msg = create_cipher(pad(b"hi alice", BLOCK_SIZE), str(bob_key), iv)

    print("Alice's cipher:", a_msg)
    print("Bob's cipher:", b_msg, "\n")
